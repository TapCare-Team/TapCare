...
import { POST } from "@/app/api/v1/admin/follow-up-signals/[signalId]/review/route";
...
async function postReview(body: unknown, user: SessionUser | null = buildUser()) {
  ...
  return POST(new Request("http://localhost/api/v1/admin/follow-up-signals/signal-1/review", {
    method: "POST",
    headers: {
...
describe("POST /api/v1/admin/follow-up-signals/[signalId]/review", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
